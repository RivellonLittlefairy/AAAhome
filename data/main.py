...
def writeToDatabase(db, name, coverImage, price, releaseDate, detailsPage):
    ...
    name = name.replace('\'', '\'\'')
    cursor = db.cursor()
    sql = 'select price_now from game_info_steam where game_name=\'' + name + '\''
    cursor.execute(sql)
    prices = cursor.fetchall()
    if len(prices) == 0:
        # 假如没有这个商品的记录，则插入一条记录最后一个字段值是当前时间戳
        sql = 'INSERT INTO game_info_steam (game_name, coverimage_url, price_now,price_lowest,relase_date, detail_page,nearly_lowest_timestamp)VALUES('
        ...
        if int(price) < list(prices[0])[0]:
            try:
                sql = 'update game_info_steam set price_now=\'' + price + '\' where game_name=\'' + name + '\''
                cursor.execute(sql)
                sql = 'select price_lowest from game_info_steam where game_name=\'' + name + '\''
                cursor.execute(sql)
                lowestPrice = cursor.fetchall()
                # 如果当前价格是史低
                if list(lowestPrice[0])[0] >= int(price):
                    sql = 'update game_info_steam set price_lowest=\'' + price + '\' where game_name=\'' + name + '\''
                    cursor.execute(sql)
                    sql = 'update game_info_steam set nearly_lowest_timestamp=\'' + str(
